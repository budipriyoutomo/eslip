...
class Tmsteslipxft extends MY_Controller {
    ...
    function __construct()
    {
        ...
        $this->load->model('Tmsteslipxft_model');
    ...
    {
	    $tmsteslip = $this->Tmsteslipxft_model->get_all();

        $data = array(
            'action' => site_url('tmsteslipxft/proses'),
        ...
        $this->load->view('tmsteslipxft/tmsteslip_head',$data);
        $this->load->view('tmsteslipxft/tmsteslip_list',$tmsteslip);
        $this->load->view('footer');
    }

    public function json() {
        header('Content-Type: application/json');
        echo $this->Tmsteslipxft_model->json();
    }
            ...
            if ( ! $this->upload->do_upload()) {
              ...
              for ($i = 1; $i <= $data['numRows']; $i++) {
                  ...
                  // $dataexcel[$i - 1]['periode'] = $data['cells'][$i][4];
                   ...
                   $dataexcel[$i - 1]['bonus'] = $data['cells'][$i][6];
                   $dataexcel[$i - 1]['lembur'] = $data['cells'][$i][7];
                   $dataexcel[$i - 1]['uangmakan'] = $data['cells'][$i][8];
                   $dataexcel[$i - 1]['transport'] = $data['cells'][$i][9];
                   $dataexcel[$i - 1]['pengmcu'] = $data['cells'][$i][10];
                   $dataexcel[$i - 1]['bpjsnakerjht'] = $data['cells'][$i][11];
                   $dataexcel[$i - 1]['bpjsnakerjp'] = $data['cells'][$i][12];
                   $dataexcel[$i - 1]['bpjssehat'] = $data['cells'][$i][13];
                   $dataexcel[$i - 1]['potmcu'] = $data['cells'][$i][14];
                   $dataexcel[$i - 1]['potlain'] = $data['cells'][$i][15];
                   $dataexcel[$i - 1]['ketpot'] = $data['cells'][$i][16];
                   $dataexcel[$i - 1]['penlain'] = $data['cells'][$i][17];
                   $dataexcel[$i - 1]['ketpen'] = $data['cells'][$i][18];
                   $dataexcel[$i - 1]['sakit'] = $data['cells'][$i][19];
                   $dataexcel[$i - 1]['alpha'] = $data['cells'][$i][20];
                   $dataexcel[$i - 1]['cutdak'] = $data['cells'][$i][21];
                   $dataexcel[$i - 1]['hadir'] = $data['cells'][$i][22];
                   $dataexcel[$i - 1]['thp'] = $data['cells'][$i][23];
                   //$dataexcel[$i - 1]['penerbitgaji'] = $data['cells'][$i][24];
                   $dataexcel[$i - 1]['penerbitgaji'] = date("Y-m-d",strtotime(substr($this->input->post('penerbitgaji',TRUE),0,10)));//$data['cells'][$i][24];
                   //$dataexcel[$i - 1]['namapt'] = $data['cells'][$i][25];
                   $dataexcel[$i - 1]['namapt'] = $this->input->post('namapt',TRUE);//$data['cells'][$i][25];
                   $dataexcel[$i - 1]['email'] = $data['cells'][$i][26];

              }

              //load model
              $this->Tmsteslipxft_model->loaddata($dataexcel);
              ...
              redirect(site_url('Tmsteslipxft'));
            }
    ...
    function genpdf(){
        ...
        if($idpt==1){
          ...
          $pdf->Image('./image/logo-XF.png',10,10,-300); // code...
        }
      ...
      //  $pdf->Cell(190,6,'Periode : Desember 2018 ',0,1,'R');
        ...
        $pdf->Cell(55,5,'BPJS KETENAGAKERJAAN JHT',0,0,'L');
        $pdf->Cell(40,5,' = '.number_format($row->bpjsnakerjht,2,",","."),0,1,'L');

        $pdf->Cell(40,5,'BONUS',0,0,'L');
        $pdf->Cell(40,5,' = '.number_format($row->bonus,2,",","."),0,0,'L');
        $pdf->Cell(55,5,'BPJS KETENAGAKERJAAN JP',0,0,'L');
        $pdf->Cell(40,5,' = '.number_format($row->bpjsnakerjp,2,",","."),0,1,'L');
        ...
        $pdf->Cell(40,5,' = '.number_format($row->pengmcu,2,",","."),0,0,'L');
        $pdf->Cell(55,5,'KET POTONGAN :',0,1,'L');

        $pdf->Cell(40,5,'PENERIMAAN LAINNYA',0,0,'L');
        $pdf->Cell(40,5,' = '.number_format($row->penlain,2,",","."),0,0,'L'); 
        $pdf->Cell(80,5,$row->ketpot,0,1,'L');
        
       

        $pdf->Cell(40,5,'KET PENERIMAAN : ',0,0,'L');
        $pdf->Cell(40,5,'',0,0,'L');
        $pdf->Cell(40,5,'JUMLAH SAKIT',0,0,'L');
        $pdf->Cell(55,5,' = '.$row->sakit,0,1,'L');
        $pdf->Cell(40,5,$row->ketpen,0,0,'L');
        $pdf->Cell(40,5,'',0,0,'L');
    
        //$pdf->Cell(80,5,$row->ketpen,0,0,'L');
        ...
        $pdf->Cell(55,5,' = '.$row->cutdak,0,1,'L');
        $pdf->Cell(80,3,'',0,0);
        $pdf->Cell(40,5,'JUMLAH KEHADIRAN',0,0,'L');
        $pdf->Cell(55,5,' = '.$row->hadir,0,1,'L');
        ...
        $totpenerimaan = $row->gaji + $row->bonus + $row->lembur +  $row->uangmakan + $row->transport + $row->pengmcu + $row->penlain;
        $pdf->Cell(40,6,' = '.number_format($totpenerimaan,2,",","."),0,0,'L');
        $pdf->Cell(55,6,'TOTAL POTONGAN',0,0,'L');
        $totpotongan = $row->bpjsnakerjht + $row->bpjsnakerjp + $row->bpjssehat + $row->potmcu +  $row->potlain ;
        ...
        $pdf->SetFont('Arial','',8);
        $output = './temp_send/'.$row->nik.'.pdf';
        $pdf->Output($output,'F');
        //$pdf->Output();
        //$email =$row->email;

        $this->setemail($row->email,$brand->namaPT,$periode,$output);
         $this->load->helper('file');
         unlink($output);
         delete_files($output);
        $this->Tmsteslipxft_model->delete($row->id);
    ...
    public function setemail($email,$pt,$periode,$output)
  	{
  		//	$email="budi_p_u@yahoo.co.uk";
  			$subject="E-Slip ".$pt;
  			$message="Dear $email, Berikut E-Slip Gaji Bulan ".$periode;
  			$this->sendEmail($email,$subject,$message,$output);
  	}

    public function sendEmail($email,$subject,$message,$output)
    {
    ...
    public function delete($id)
    {
		...
		if ($level!='User')
		{
			$row = $this->Tmsteslipxft_model->get_by_id($id);

			if ($row) {
				$this->Tmsteslipxft_model->delete($id);
    ...
    public function deldata(){
      $level = $this->session->userdata('role') ;
		...
		if ($level!='User')
		{
      	$this->Tmsteslipxft_model->truncateall();
        $this->session->set_flashdata('message', 'Delete Record Success');
				redirect(site_url('tmsteslip'));
    }
  ...
  $this->form_validation->set_rules('bpjsnakerjht', 'bpjsnakerjht', 'trim|required|numeric');
  $this->form_validation->set_rules('bpjsnakerjp', 'bpjsnakerjp', 'trim|required|numeric');
